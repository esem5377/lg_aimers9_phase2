"""jh_ws v20_control_risk_score(982 레시피 + control_risk_score/_weighted
피처, 실제 리더보드 992 확인됨)를 새 베이스로 삼아, tune_arch_blend_honest.py의
아키텍처 블렌드(CatBoost+LGBM+XGBoost)를 이 피처셋 위에서 다시 fold0(->2022)
/fold2(->2024) walk-forward + calib_fit/calib_eval 정직 분리로 검증한다.

주의: v20의 로컬 검증(iterations=1000, 8/22 세션)은 fold0 -3.74/fold2 +7.04로
방향이 엇갈렸는데도 실제 리더보드에서 982->992로 뚜렷이 개선됐음 -- 이
프로젝트에서 드물게 "혼재된 로컬 신호가 실제로는 양수였던" 사례. 그만큼 이
피처셋 위에서의 블렌드 가중치도 로컬 신호만으로 과신하지 말고 실측 전제로
접근할 것.
"""
import json
import os
...
def run_fold(fold, df, id_mappings):
    ...
    for name, raw in raw_all.items():
        ...
        ab[name] = (a, b)
        fit_preds[name] = apply_platt_scaling(raw.loc[idx_fit], a, b)
    best_w = grid_search_blend(fit_preds, y_fit)

    eval_preds = {name: apply_platt_scaling(raw.loc[idx_eval], *ab[name]) for name, raw in raw_all.items()}
    single_bss_eval = {k: bss_score(v, y_eval) for k, v in eval_preds.items()}
    single_best_name = max(single_bss_eval, key=single_bss_eval.get)
    single_best_bss = single_bss_eval[single_best_name]

    blend_eval = sum(best_w[k] * eval_preds[k] for k in ("cat", "lgb", "xgb"))
    ...
    return {
        "fold": fold["name"], "blend_weights_from_calib_fit": best_w,
        "single_bss_eval": single_bss_eval, "single_best_name": single_best_name,
        "single_best_bss_eval": single_best_bss, "blend_bss_eval": blend_bss_eval,
        "delta_honest": delta, "elapsed_sec": dt,
    }


def main():
    t0 = time.time()
    print("Load data (+control_risk_score 피처)...")
    df = load_data()
    ...
    with open(os.path.join(OUT_DIR, "tune_arch_blend_honest_riskscore_results.json"), "w", encoding="utf-8") as f:
        json.dump({"folds": results, "both_positive": both_positive}, f, indent=2)
    print(f"\nSaved: {OUT_DIR}/tune_arch_blend_honest_riskscore_results.json")
