"""v3 실험(6시드 CatBoost + 신규 NCA 재학습)이 2082.78(v26, 1시드)을 못 넘은
원인이 NCA 재학습 자체의 GPU 비결정성(1845 vs 원본 1897)으로 의심되어,
NCA는 재학습하지 않고 jh_ws v26의 검증된 retrieval_encoder.pt를 그대로
로드해서 임베딩만 다시 계산(reference_embeddings.npy는 용량 때문에
.gitignore되어 있어 커밋 안 됨 -- encoder 가중치로 재계산 가능, 순전파라
결정적)하고, 여기에 새로 학습한 CatBoost 6시드 평균만 블렌드해 6시드 배깅
자체의 순수 효과를 분리 측정한다.

carve-out split(5%, stratify, random_state=42)이 jh_ws와 동일한 레시피/
train.csv로 재현되므로 train_sub_df가 bit-identical할 것으로 기대(6시드
CB-only 평균 BSS 2045.73이 v25 1시드 CB-only 2045.96과 거의 일치해 이미
확인됨).
"""
import json
import os
...
CB6_MODEL_DIR = os.path.join(_BASE, "model_retrieval_blend_v3")
...
CATBOOST_CAT_COLS = ["top_bottom", "game_type", "base_state", "pitcher_hand", "batter_hand",
...
def main():
    log("Load train data (v22/25/26과 동일 레시피)...")
    df = load_data()
    ...
    log("\n=== 저장된 CatBoost 6시드 로드 + calib 예측(재학습 없음) ===")
    cb_calib_raws = []
    for seed in CB_SEEDS:
        m = CatBoostClassifier()
        m.load_model(os.path.join(CB6_MODEL_DIR, f"catboost_seed{seed}.cbm"))
        cb_calib_raws.append(m.predict_proba(X_calib_cb)[:, 1])
    cb_calib_raw = np.mean(cb_calib_raws, axis=0)
    log(f" CatBoost 6시드 평균 carve-out BSS(raw)={bss_score(cb_calib_raw, y_calib):.2f}")

    log("\n=== v26 retrieval_encoder.pt 로드(재학습 없음) + 임베딩 재계산 ===")
    ...
    encoder.eval()
    log(f" device={device}  cardinalities_match={meta_rt['cardinalities']}")
    ...
    log(f" Retrieval(v26 encoder 재사용) carve-out BSS(raw)={nca_bss:.2f}  "
        f"(원본 v25 단독 1896.79와 비교, 재학습판 1845.04보다 얼마나 근접한지 확인)")

    log("\n=== 블렌드 가중치 재탐색 (2-way, CB=6시드) ===")
    best = None
    for i in range(21):
        w_cb = i * 0.05
        raw = w_cb * cb_calib_raw + (1 - w_cb) * nca_calib_raw
        score = bss_score(raw, y_calib)
        if best is None or score > best[0]:
            best = (score, w_cb)
    best_raw_bss, best_w_cb = best
    raw_best = best_w_cb * cb_calib_raw + (1 - best_w_cb) * nca_calib_raw
    a, b = fit_platt_scaling(raw_best, y_calib)
    calib_best = apply_platt_scaling(raw_best, a, b)
    calib_bss = bss_score(calib_best, y_calib)
    log(f" best w_cb={best_w_cb:.2f}  raw_bss={best_raw_bss:.2f}  calibrated_bss={calib_bss:.2f}")

    raw_07 = 0.7 * cb_calib_raw + 0.3 * nca_calib_raw
    a7, b7 = fit_platt_scaling(raw_07, y_calib)
    calib_07 = apply_platt_scaling(raw_07, a7, b7)
    log(f" [고정 0.7:0.3] calibrated_bss={bss_score(calib_07, y_calib):.2f}")

    result = {
        "cb6_only_raw_bss": bss_score(cb_calib_raw, y_calib),
        "nca_v26encoder_reused_raw_bss": nca_bss,
        "best_2way": {"w_cb": best_w_cb, "raw_bss": best_raw_bss, "calibrated_bss": calib_bss, "a": a, "b": b},
        "fixed_07_03": {"calibrated_bss": bss_score(calib_07, y_calib), "a": a7, "b": b7},
        "reference_v26_1seed_2way_calibrated": 2082.7818915293506,
        "reference_v3_retrained_nca_2way_calibrated": 2071.6551523424996,
    }
    log(f"\n=== 최종 비교 ===")
    log(f" v26(1시드 CB, 원본 encoder): 2082.78")
    log(f" v3(6시드 CB, 재학습 encoder): 2071.66")
    log(f" v4(6시드 CB, v26 encoder 재사용, 최적가중치 w_cb={best_w_cb:.2f}): {calib_bss:.2f}")
    log(f" delta(v4 - v26) = {calib_bss - 2082.7818915293506:+.2f}")

    with open(os.path.join(OUT_DIR, "metrics_reuse_v26_retrieval_cb6.json"), "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, ensure_ascii=False)
    log(f"\nSaved metrics to {OUT_DIR}/metrics_reuse_v26_retrieval_cb6.json")
