# script.py
# v26(1023점, CatBoost:retrieval=0.7:0.3) 구조를 그대로 재사용하되, CatBoost의
# loss_function만 Logloss -> RMSE(CatBoostRegressor)로 교체. fold0(train<=2021->eval=2022)
# +14.96/+8.65, 블렌드 안에서 +11.28/+5.28 walk-forward 개선을 근거로 함(랜덤 carve-out은
# 반대로 -8.74였으나, 이 프로젝트 방법론상 walk-forward를 우선 신뢰).
# retrieval encoder/reference_embeddings는 v26 것을 그대로 재사용(재학습 없음).
import json
import os
...
def main():
    ...
    print("CatBoost inference (RMSE regressor)...")
    ...
    cb_raw = cb_model.predict(X_cb) if len(X_cb) else np.array([])
