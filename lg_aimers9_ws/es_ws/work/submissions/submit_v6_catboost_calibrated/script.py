# script.py
# CatBoost + sigmoid 확률보정(Platt scaling) 버전 — 리더보드 점수 확인용.
# submit_v4_catboost(889점, 보정 없음) 대비 유일한 변경점: 예측 확률에
# feature_meta.json의 calibration.a/b로 Platt scaling(1/(1+exp(-(a*p+b))))
# 적용. sklearn CalibratedClassifierCV를 직접 pickle하지 않고 스칼라
# 2개(a, b)만 저장/적용하므로 버전 의존성 없음(8/16 pandas 크로스버전
# unpickle 사고와 같은 클래스의 리스크를 원천적으로 피함).
# 로컬 2024 홀드아웃 검증: BSS 800.75 -> 825.57 (+24.8), AUC는 monotonic
# 변환이라 0.55056으로 그대로(보정은 확률의 절대적 정확도만 개선, 순위는 불변).
import json
import os
