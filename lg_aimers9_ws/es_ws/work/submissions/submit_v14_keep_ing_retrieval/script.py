# script.py
# CatBoost는 원재료(asof_pitcher_reverse/middle/ball_rate) 유지 버전(jh_ws
# v20, 6시드 배깅) + retrieval은 jh_ws v26의 검증된 인코더 그대로 재사용.
# 같은 원재료를 유지한 CatBoost가(2056.07 raw) 제거한 버전(2045.73 raw)보다
# 6시드 기준 오히려 더 강했음 -- v22/v25/v26이 원재료를 제거한 게 1시드
# 비교(977 vs 987)에서 다른 피처 차이와 뒤섞여 나온 결론이었을 가능성.
# carve-out calibrated BSS: v26(1시드,drop)=2082.78 -> v13(6시드,drop)=2085.15
# -> v14(6시드,KEEP)=2090.34.
import json
import os
...
def main():
    ...
    w_cb = meta.get("blend_weight_catboost", 0.7)

    cb_models = []
    for seed in meta_cb["seeds"]:
        cb = CatBoostClassifier()
        cb.load_model(os.path.join(MODEL_DIR, f"catboost_seed{seed}.cbm"))
        cb_models.append(cb)
    print(f" CatBoost {len(cb_models)}시드 로드 완료: seeds={meta_cb['seeds']}")
    ...
    state_dict = torch.load(os.path.join(MODEL_DIR, "retrieval_encoder.pt"), map_location=device, weights_only=True)
    ...
    test = add_control_risk_score(test)
    ids = test[ID_COL].tolist()

    print("CatBoost inference (6시드 평균, 원재료 유지)...")
    ...
    if len(X_cb):
        cb_raw = np.mean([cb.predict_proba(X_cb)[:, 1] for cb in cb_models], axis=0)
    else:
        cb_raw = np.array([])

    print("Retrieval inference (원재료 제거 피처셋)...")
    if len(test):
        test_rt = build_retrieval_input(test)
        cat_query = encode_cats(test_rt, meta_rt["cat_mappings"])
        x_num_query = prep_numeric_transform(test_rt, meta_rt["numeric_cols"], medians, scaler)
