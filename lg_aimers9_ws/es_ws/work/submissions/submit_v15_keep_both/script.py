# script.py
# CatBoost 6시드(원재료 유지, jh_ws v20 자산 재사용) + retrieval도 원재료
# 유지 피처셋으로 재학습(단일 시드=7, cudnn deterministic 모드).
# carve-out calibrated BSS: v26(1023 실측,1시드,drop)=2082.78 -> v13(6시드,
# drop)=2085.15 -> v14(6시드 CB-keep+drop retrieval)=2090.34 -> v15(6시드
# CB-keep + retrieval-keep 단일시드)=2098.71 (v26 대비 +15.93).
# retrieval 3시드 앙상블(2096.73)도 시도했으나 단일시드가 더 나았고, 추론
# 1회로 끝나 서버 10분 제한 리스크도 3배 안 늘어남 -- 단일시드 채택.
import json
import os
...
def encode_cats(df, mappings):
    ...
    for col, mp in mappings.items():
        oov = len(mp)
        out[col] = df[col].astype(str).map(mp).fillna(oov).astype(int).values
    return out
...
def main():
    ...
    w_cb = meta.get("blend_weight_catboost", 0.65)
    ...
    print("Retrieval inference (원재료 유지 피처셋, 단일시드 인코더)...")
    if len(test):
