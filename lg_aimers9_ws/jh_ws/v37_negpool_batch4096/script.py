# script.py (v25/v35와 동일, 아키텍처를 전부 feature_meta.json에서 읽어오는
# 범용 추론 스크립트라 변경 없이 재사용)
# v37: CatBoost는 v26 것 재사용, retrieval encoder만 in-batch negative pool
# 확대(BATCH_SIZE 1024->4096, 나머지 용량/구조는 v25/v26과 동일)로 전체
# 프로덕션 데이터 재학습. walk-forward 검증 결과를 기다리지 않고 사용자
# 요청으로 바로 프로덕션 빌드 진행(01_build_v37.py 상단 docstring 참고).
# 가중치는 feature_meta.json의 blend_weight_catboost 필드에서 읽음.
import json
import os
