# script.py
# retrieval(NCA) 점수를 CatBoost와 사후 블렌드하지 않고, CatBoost의 새 입력
# 피처로 추가한 단일 모델. train_sub 자신의 retrieval_score는 K=3 OOF로
# 생성해 학습에 썼고(build_oof_and_train.py), 여기 추론 시점(test)에는 v26의
# encoder+참조집합(140만행, 재학습 없음)을 그대로 재사용해 retrieval_score를
# 계산한 뒤 CatBoost 입력에 포함시킨다.
import json
import os
...
def apply_calibration(raw_p, calib):
    if calib is None:
        return raw_p
    method = calib.get("method", "platt_sigmoid")
    if method == "beta":
        eps = 1e-6
...
def build_catboost_features(df, retrieval_score, meta):
...
def main():
    ...
    n_numeric = len(rt["numeric_cols"]) * 2  # 원본 + isna 플래그
    encoder = RowEncoder(
        rt["cardinalities"], rt["embed_dims"], rt["encoder_hidden"], rt["embed_out_dim"], 0.0, n_numeric,
    ).to(device)
    ...
    print(f" OK. n_features={len(meta['columns'])}  reference_size={len(y_ref)}  calibration={calib}")
    ...
    print("Retrieval score 계산(v26 encoder+참조집합 재사용)...")
    if len(test):
        cat_query = encode_cats(test, rt["cat_mappings"])
        x_num_query = prep_numeric_transform(test, rt["numeric_cols"], medians, scaler)
        retrieval_score = retrieve_predict(encoder, device, cat_query, x_num_query, z_ref, y_ref)
    else:
        retrieval_score = np.array([])

    print("CatBoost inference (retrieval_score 피처 포함)...")
    X_cb = build_catboost_features(test, retrieval_score, meta)
    n_oov_pitcher = int((X_cb["pitcher_id"] == -1).sum()) if len(X_cb) else 0
    n_oov_batter = int((X_cb["batter_id"] == -1).sum()) if len(X_cb) else 0
    ...
    preds = apply_calibration(cb_raw, calib) if len(X_cb) else []
