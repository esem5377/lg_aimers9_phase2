...
# 그대로 재사용 -- 재학습 없이 calibration 방식만 교체. v26은 "블렌드 후
# Platt 1회"였는데, CatBoost/retrieval을 각각 isotonic으로 개별 보정한 뒤
# 블렌드하는 방식이 production 규모 calib holdout(20%)에서 +15.23으로 더
# 나음을 확인해 채택(calibration_compare.py). isotonic은 (a,b) 스칼라로
# 재현 불가능해 sklearn 객체를 pickle하는 대신(8/16 sklearn 버전 크로스
# 호환 사고 전례로 회피) X/y threshold 배열만 저장, 여기서는 순수
# numpy(np.interp)로 재현 -- sklearn IsotonicRegression(out_of_bounds="clip")
# 과 수학적으로 동일(구간별 선형보간 + 경계값 클리핑).
import json
import os
...
def apply_isotonic(raw_p, iso_spec):
    """sklearn IsotonicRegression(out_of_bounds="clip")과 동일(구간별
    선형보간 + 경계값 클리핑) -- np.interp가 정확히 그 동작이라 재현 가능."""
    xp = np.asarray(iso_spec["x_thresholds"])
    fp = np.asarray(iso_spec["y_thresholds"])
    return np.interp(np.asarray(raw_p), xp, fp)
...
def main():
    ...
    iso_meta = meta["calibration_isotonic_per_model"]
    ...
    print(f" OK. reference_size={len(y_ref)}  calibration=isotonic_per_model  blend_weight_catboost={w_cb}")
    ...
    if len(test):
        cb_cal = apply_isotonic(cb_raw, iso_meta["catboost"])
        nca_cal = apply_isotonic(nca_raw, iso_meta["retrieval"])
        preds = w_cb * cb_cal + (1 - w_cb) * nca_cal
    else:
        preds = []
