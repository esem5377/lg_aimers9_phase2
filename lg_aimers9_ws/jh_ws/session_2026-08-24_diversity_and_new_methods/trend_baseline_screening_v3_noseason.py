...
RESULT_PATH = os.path.join(OUT_DIR, "trend_baseline_screening_v3_noseason_results.json")
...
def build_features(df, id_mappings, drop_season=False):
...
def run_fold(fold_name, train_df, eval_df, cache_path):
    ...
    X_train = build_features(train_sub_df, id_mappings, drop_season=True)
    X_calib = build_features(calib_df, id_mappings, drop_season=True)
    X_eval = build_features(eval_df, id_mappings, drop_season=True)
