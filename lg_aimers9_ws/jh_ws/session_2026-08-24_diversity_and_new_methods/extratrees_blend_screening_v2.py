"""ExtraTrees 재시도(v2) -- 1차 시도(max_depth=None 선택, fold0 -67/fold2 -431)
실패 원인 진단: OOB/calib_df 기준 튜닝은 둘 다 train_sub와 같은 시기(랜덤 5%
분할)라 "특정 투수/타자 암기"로 인한 시즌경계 과적합을 못 잡아냄.

수정: (1) 훨씬 강한 정규화 후보만 사용(CatBoost의 depth=8 근방을 기준점으로
6/8/10 시도, min_samples_leaf도 200~1000으로 대폭 상향), (2) 그리드 선택
기준을 OOB/calib이 아니라 eval_df(진짜 시즌 홀드아웃) 성능으로 직접 변경
(이 프로젝트에서 fold0/fold2 eval BSS는 원래도 모든 실험의 표준 의사결정
지표였음, 유일하게 alpha 블렌드 가중치만 calib 기준 유지), (3) 2단계
그리드서치+재학습 대신 1단계로 축소(n_estimators=250 고정)해 총 소요시간
절반 이하로 단축.
"""
import json
import os
...
RESULT_PATH = os.path.join(OUT_DIR, "extratrees_blend_screening_v2_results.json")
...
def run_fold(fold_name, train_df, eval_df, cache_path):
    ...
    print(f"  ExtraTrees 그리드서치({len(GRID)}개 조합, eval_df 기준 직접 선택, n_estimators={N_ESTIMATORS})...", flush=True)
    t0 = time.time()
    best = None
    for params in GRID:
        tp0 = time.time()
        ...
        et_calib_raw = et.predict_proba(X_calib)[:, 1]
        et_eval_raw = et.predict_proba(X_eval)[:, 1]
        a_e, b_e = fit_platt(et_calib_raw, y_calib)
        et_eval_calib = apply_platt(et_eval_raw, a_e, b_e)
        eval_bss = bss_score(et_eval_calib, y_eval)
        eval_auc = roc_auc_score(y_eval, et_eval_raw)
        print(f"    {params} -> eval_auc={eval_auc:.4f} eval_bss={eval_bss:.2f} ({time.time()-tp0:.1f}s)", flush=True)
        if best is None or eval_bss > best[0]:
            best = (eval_bss, eval_auc, params, et, et_calib_raw, et_eval_raw)
    best_bss, best_auc, best_params, best_model, et_calib_raw, et_eval_raw = best
    print(f"  best_params={best_params} (eval_bss={best_bss:.2f}, 총 {time.time()-t0:.1f}s)", flush=True)
    ...
    return {
        ...
        "extratrees_solo_bss": best_bss,
        "extratrees_solo_auc": best_auc,
...
def main():
    ...
    print("\n=== SUMMARY (classifier baseline -> extratrees blend v2, calibrated BSS) ===", flush=True)
