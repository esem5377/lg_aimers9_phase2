"""jh_ws/train.py 기반 실험 버전 (v5) — train_p_col 버그 수정판.
원본은 `[c for c in train_df.columns if 'pitcher' in c.lower()][0]`가
`pitcher_id`가 아니라 `score_diff_pitcher_team`에 매칭되는 버그가 있었음
(section 8 워크로그 참고). 여기서는 진짜 `pitcher_id`로 그룹핑해서
"진짜" 투수 단위 group k-fold를 재현. raw id는 원본처럼 유지, 트랙맨도
원본처럼 깨진 채(비교 기준을 909점 버전과 최대한 맞추기 위함).
"""
import os
import joblib
import random
...
print("[6/6] v5(진짜 pitcher_id group k-fold, raw id 유지) 학습 완료!")
