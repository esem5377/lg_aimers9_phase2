# script.py (v34)
# v26(CatBoost:retrieval=0.7:0.3, 팀 최고 1023점)의 CatBoost/retrieval encoder/
# 참조임베딩을 그대로 재사용(재학습 없음), 세 번째 재료로 EB-GLMM(최근1시즌만
# 재학습한 레짐분리 버전)을 저자유도(가중치 1개, w_ebglmm) 블렌드로 추가.
# blend_raw = (1-w_eb)*(w_cb*cb_raw + (1-w_cb)*nca_raw) + w_eb*eb_raw
# -> 이 blend_raw 하나에만 Platt calibration 적용(production 표준 방식).
# 가중치(w_cb=0.7, w_eb)는 전체 이력 5% calib carve-out에서 그리드서치로 선택.
import json
import os
...
def ebglmm_transform(df, state):
    numeric_cols = state["numeric_cols"]
    num_median = pd.Series(state["num_median"])
    num_mean = pd.Series(state["num_mean"])
    num_std = pd.Series(state["num_std"])
    num = df[numeric_cols].fillna(num_median)
    num = (num - num_mean) / num_std
    parts = [num.reset_index(drop=True)]
    for c in state["cat_fixed_cols"]:
        cats = state["cat_categories"][c]
    ...
    X = X.reindex(columns=state["columns_"], fill_value=0.0)
    return X.astype(float)


def ebglmm_predict_raw(df, state):
    df2 = add_count_base(df).reset_index(drop=True)
    Xd = np.hstack([np.ones((len(df2), 1)), ebglmm_transform(df2, state).to_numpy(dtype=float)])
    eta = Xd @ state["beta"]
    for gcol in state["groups"]:
        gmap = state["group_maps"][gcol]
        codes = df2[gcol].map(gmap)
        valid = codes.notna().to_numpy()
        add = np.zeros(len(df2))
        idx = codes[valid].astype(int).to_numpy()
        add[valid] = state["u_effects"][gcol][idx]
        eta += add
...
def main():
    ...
    w_eb = meta.get("blend_weight_ebglmm", 0.0)
    ...
    ebglmm_state = joblib.load(os.path.join(MODEL_DIR, "ebglmm_state.pkl"))

    context = joblib.load(CONTEXT_PATH)
    print(f" OK. reference_size={len(y_ref)}  calibration={calib}  "
          f"blend_weight_catboost={w_cb}  blend_weight_ebglmm={w_eb}")
    ...
    if len(test):
        ...
        nca_raw = retrieve_predict(encoder, device, cat_query, x_num_query, z_ref, y_ref)
    else:
        nca_raw = np.array([])

    print("EB-GLMM inference...")
    eb_raw = ebglmm_predict_raw(test, ebglmm_state) if len(test) else np.array([])

    print("Build submission...")
    if len(test):
        base_raw = w_cb * cb_raw + (1 - w_cb) * nca_raw
        blend_raw = (1 - w_eb) * base_raw + w_eb * eb_raw
