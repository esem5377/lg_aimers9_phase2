...
print("\n=== 블렌드 가중치 세밀 그리드서치 (0.5~0.85, 0.05 간격) ===", flush=True)
results = []
for w in np.arange(0.5, 0.86, 0.05):
    ...
    results.append((round(w, 2), bss))
    print(f"  w={w:.2f}  (catboost={w:.2f} / retrieval={1-w:.2f})  calibrated BSS={bss:.2f}", flush=True)

best_w, best_bss = max(results, key=lambda r: r[1])
print(f"\n최적 가중치: w={best_w} (catboost={best_w}/retrieval={round(1-best_w,2)}), BSS={best_bss:.2f}", flush=True)
print(f"참고: w=0.50(기존 v25) BSS={dict(results)[0.5]:.2f}, w=0.70(v26) BSS={dict(results)[0.7]:.2f}", flush=True)

with open("blend_weight_search_fine_results.json", "w", encoding="utf-8") as f:
    json.dump({"results": results, "best_w": best_w, "best_bss": best_bss}, f, indent=2, ensure_ascii=False)
print("Saved: blend_weight_search_fine_results.json", flush=True)
