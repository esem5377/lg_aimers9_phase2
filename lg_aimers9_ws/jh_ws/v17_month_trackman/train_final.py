"""974점(es_ws v7) 레시피 + trackman_context에 4번째 독립 축(game_month)
추가. 나머지(CAT_COLS/RAW_ID_COLS/BEST_PARAMS/Platt calibration)는 전부
동일. iterations=2000(진짜 프로덕션 스케일). GroupKFold는 시간 문제로
스킵, 레시피 내장 season 분할 진단만 확인 후 바로 제출까지 진행.
"""
import json
import os
...
TRACKMAN_CONTEXT_PATH = (
    r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v17_month_trackman\model\trackman_context.pkl"
)
MODEL_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v17_month_trackman\model"
OUT_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v17_month_trackman\output"
...
def main():
    ...
    print("Load train data (month_state trackman context)...")
    df = load_data()
    ...
    print(f" 참고: 원래 974 레시피(3축)의 동일 진단 BSS는 805.35였음")
    with open(os.path.join(OUT_DIR, "metrics_v17_month.json"), "w", encoding="utf-8") as f:
