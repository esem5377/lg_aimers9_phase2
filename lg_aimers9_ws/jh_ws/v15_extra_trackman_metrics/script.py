# script.py
# 974점(es_ws v7)과 동일 추론 코드. 변경점은 model/trackman_context.pkl이
# METRIC_COLS 4->8개(extension/rel_height/rel_side/zone_speed 추가,
# 1%/99% winsorize)로 재생성된 버전이라는 것뿐 -- 추론 로직 자체는 무변경.
import json
import os
...
def main():
    ...
    print(f" OK. calibration={calib}  raw_id_cols={meta.get('raw_id_cols')}  n_columns={len(meta.get('columns', []))}")
