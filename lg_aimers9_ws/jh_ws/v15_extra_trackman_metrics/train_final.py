"""974점(es_ws v7) 레시피 + trackman_context METRIC_COLS 4->8개 확장
(extension/rel_height/rel_side/zone_speed 추가, 1%/99% winsorize 적용).
그룹핑 구조(count_state/hand_matchup/inning_state)와 나머지 레시피
(CAT_COLS/RAW_ID_COLS/BEST_PARAMS/Platt calibration)는 전부 동일.

iterations=2000 -- 진짜 프로덕션 스케일. GroupKFold 별도 검증은 시간
문제로 스킵하고, 레시피에 내장된 season 분할 진단(model_cv)만으로 1차
확인 후 바로 제출까지 진행.
"""
import json
import os
...
TRACKMAN_CONTEXT_PATH = (
    r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v15_extra_trackman_metrics\model\trackman_context.pkl"
)
MODEL_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v15_extra_trackman_metrics\model"
OUT_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v15_extra_trackman_metrics\output"
...
def main():
    ...
    print("Load train data (extended trackman context)...")
    df = load_data()
    ...
    print(f" train={X_tr.shape} valid(2024)={X_va.shape}  n_features={X_all.shape[1]}")
    ...
    print(f" 참고: 원래 974 레시피(4개 METRIC_COLS)의 동일 진단 BSS는 805.35였음")
    with open(os.path.join(OUT_DIR, "metrics_v15_extra_metrics.json"), "w", encoding="utf-8") as f:
