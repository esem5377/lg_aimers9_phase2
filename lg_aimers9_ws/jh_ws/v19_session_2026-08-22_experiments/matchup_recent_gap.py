"""992점(v20 control_risk_score) 세션에서 새로 찾은 두 후보 검증.
연도별 안정성 스크리닝(사전 필터)에서 원재료보다 뚜렷이 안정해진(부호flip 없음)
2개만 통과시켰음:
  matchup_skill_gap = asof_pitcher_success_rate - asof_batter_success_rate
    (원재료 중 batter_success_rate가 range=0.089로 매우 불안정했는데,
     차이를 취하면 range=0.030으로 크게 안정화됨)
  recent_form_gap = asof_pitcher_prev1_game_success_rate - asof_pitcher_success_rate
    (두 원재료 range 0.040/0.046 -> 차이는 range=0.027로 더 안정)

982 레시피(CAT_COLS 네이티브/RAW_ID_COLS/trackman context/BEST_PARAMS) 그대로,
baseline/+matchup/+recent_form/+both 4가지를 fold0(->2022)/fold2(->2024)
season walk-forward + 정직한 calibration(train 파티션 내부 carve-out에만
Platt fit)으로 비교.
"""
import json
import os
...
RESULT_PATH = os.path.join(OUT_DIR, "matchup_recent_gap_results.json")
...
VARIANTS = ["baseline", "matchup", "recent_form", "both"]
...
def main():
    ...
    print("\n=== SUMMARY (delta vs baseline, calibrated BSS) ===", flush=True)
