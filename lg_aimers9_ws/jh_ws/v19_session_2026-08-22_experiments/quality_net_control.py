"""992점(v20 control_risk_score) 레시피를 baseline으로 두고, 연도별 안정성
스크리닝을 통과한 2개 신규 후보를 추가 검증:
  control_quality_score = asof_pitcher_success_rate + asof_pitcher_strike_rate
    (원재료 중 strike_rate가 range=0.059, 부호flip 있었는데 합치면
     range=0.021로 크게 안정화 -- control_risk_score와 같은 성공 패턴)
  pitcher_net_control = asof_pitcher_success_rate - control_risk_score
    (success_rate 단독 range=0.046보다 안정한 0.031, 상관계수는 오히려
     0.084로 더 강해짐 -- "스킬 - 위험" net 지표)

baseline은 992 레시피 그대로(control_risk_score/_weighted 포함, 73피처).
fold0(->2022)/fold2(->2024) season walk-forward + 정직한 calibration(train
파티션 내부 carve-out에만 Platt fit).
"""
import json
import os
import time
...
RESULT_PATH = os.path.join(OUT_DIR, "quality_net_control_results.json")
...
VARIANTS = ["baseline", "quality", "net_control", "both"]
...
def add_features(df, variant):
    df = df.copy()
    # baseline == 992 레시피: control_risk_score(*2)는 항상 포함
...
def run_variant(tag, train_df, eval_df, variant):
    train_df = add_features(train_df, variant)
    eval_df = add_features(eval_df, variant)
    ...
    result = {
        "tag": tag, "variant": variant, "n_features": X_train_sub.shape[1],
...
def main():
    ...
    for fold_name, (train_df, eval_df) in fold_specs.items():
        ...
        for variant in VARIANTS:
            r = run_variant(f"{fold_name}/{variant}", train_df, eval_df, variant)
            fold_results[variant] = r
        base_bss = fold_results["baseline"]["bss_calibrated"]
        for variant in VARIANTS:
            fold_results[variant]["delta_vs_baseline"] = fold_results[variant]["bss_calibrated"] - base_bss
    ...
    for variant in VARIANTS:
        if variant == "baseline":
            continue
        deltas = {fn: all_results[fn][variant]["delta_vs_baseline"] for fn in fold_specs}
        deltas["all_axes_positive"] = all(v > 0 for v in deltas.values())
        summary[variant] = deltas
    all_results["summary"] = summary

    with open(RESULT_PATH, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    print("\n=== SUMMARY (delta vs 992-baseline, calibrated BSS) ===", flush=True)
    for variant, deltas in summary.items():
        print(f"  {variant}: {deltas}", flush=True)
