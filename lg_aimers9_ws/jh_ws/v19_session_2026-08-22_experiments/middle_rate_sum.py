"""pitcher middle_rate 계열 4개(asof_pitcher_middle_rate/prev1/prev3/prev5
_game_middle_rate)를 단순합한 pitcher_middle_sum 검증. 스크리닝 결과 4개 중
3개보다는 안정적이지만 가장 안정적인 원재료(prev1, range=0.013)보다는 못했고,
원재료끼리 상호상관도 이미 높아(prev3-prev5=0.846) control_risk_score만큼
확실한 신호는 아니었음 -- 그래도 실측 확인.

baseline(992)은 quality_net_control.py에서 이미 계산됐으므로 재사용,
middle_sum 변형만 신규 학습. sum과 avg는 스케일만 다르고 트리 분할 정보는
동일해 sum만 테스트.
...
RESULT_PATH = os.path.join(OUT_DIR, "middle_rate_sum_results.json")
...
MIDDLE_COLS = ["asof_pitcher_middle_rate", "asof_pitcher_prev1_game_middle_rate",
...
def add_middle_sum(df):
    ...
    df["pitcher_middle_sum"] = df[MIDDLE_COLS].sum(axis=1)
    return df
...
def run_variant(tag, train_df, eval_df):
    train_df = add_middle_sum(train_df)
    eval_df = add_middle_sum(eval_df)
    ...
    result = {
        "tag": tag, "variant": "middle_sum", "n_features": X_train_sub.shape[1],
...
def main():
    ...
    for fold_name, (train_df, eval_df) in fold_specs.items():
        ...
        r = run_variant(f"{fold_name}/middle_sum", train_df, eval_df)
        baseline_bss = prior[fold_name]["baseline"]["bss_calibrated"]
        all_results[fold_name] = {
            "baseline_reused": prior[fold_name]["baseline"],
            "middle_sum": r,
            "delta_vs_baseline": r["bss_calibrated"] - baseline_bss,
        }
        with open(RESULT_PATH, "w", encoding="utf-8") as f:
            json.dump(all_results, f, indent=2, ensure_ascii=False)

    summary = {fn: all_results[fn]["delta_vs_baseline"] for fn in fold_specs}
    summary["all_axes_positive"] = all(v > 0 for v in summary.values())
    all_results["summary"] = summary
    with open(RESULT_PATH, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    print("\n=== SUMMARY (middle_sum vs 992-baseline) ===", flush=True)
