...
단독(pitcher_net_control 없이)으로, 원재료(success_rate, strike_rate)를
제거한 버전을 검증. control_risk_score 자체의 원재료(reverse/middle/ball_rate)는
건드리지 않음(이미 유지가 낫다고 결론남).

baseline과 quality(원재료 유지)는 quality_net_control.py에서 이미 계산됐으므로
재사용, quality_drop_ingredients 변형만 신규 학습.
  baseline(992): fold0=2382.75, fold2=839.44
  quality(원재료 유지): fold0=2373.50(-9.26), fold2=839.10(-0.34)
"""
import json
import os
import time
...
RESULT_PATH = os.path.join(OUT_DIR, "quality_only_drop_ing_results.json")
...
def add_quality_drop_ingredients(df):
    df = df.copy()
    ...
    df = df.drop(columns=QUALITY_INGREDIENT_COLS)
    return df
...
def run_drop(tag, train_df, eval_df):
    train_df = add_quality_drop_ingredients(train_df)
    eval_df = add_quality_drop_ingredients(eval_df)
    ...
    result = {
        "tag": tag, "variant": "quality_drop_ingredients", "n_features": X_train_sub.shape[1],
...
def main():
    ...
    for fold_name, (train_df, eval_df) in fold_specs.items():
        ...
        drop_r = run_drop(f"{fold_name}/quality_drop_ingredients", train_df, eval_df)
        baseline_bss = prior[fold_name]["baseline"]["bss_calibrated"]
        quality_kept_bss = prior[fold_name]["quality"]["bss_calibrated"]
        all_results[fold_name] = {
            "baseline_reused": prior[fold_name]["baseline"],
            "quality_kept_ingredients_reused": prior[fold_name]["quality"],
            "quality_drop_ingredients": drop_r,
            "delta_drop_vs_baseline": drop_r["bss_calibrated"] - baseline_bss,
            "delta_drop_vs_quality_kept": drop_r["bss_calibrated"] - quality_kept_bss,
        }
    ...
    summary = {
        fn: {
            ...
            "delta_drop_vs_quality_kept": all_results[fn]["delta_drop_vs_quality_kept"],
        }
    ...
    print("\n=== SUMMARY (quality_drop_ingredients vs baseline / vs quality_kept) ===", flush=True)
