"""v5(진짜 group k-fold 재현, 909점 설정과 동일) + CatBoost 하이퍼파라미터 튜닝판.

tune_catboost_v5.py 랜덤서치 결과: trial8(learning_rate=0.01, depth=8,
l2_leaf_reg=20, bagging_temperature=1, random_strength=1, border_count=32)이
baseline(depth=6, 현재 프로덕션 값) 대비 5-fold 전체에서 일관되게 우세
(mean_auc 0.57032->0.57061, +0.00029, 5/5 폴드 전부 개선). LGBM/XGB는
아직 미탐색이라 원본 그대로 두고 CatBoost만 교체해서 전체 앙상블(calibration
+ Optuna 블렌드 포함) 레벨에서 실제로 개선되는지 확인한다.
"""
import os
import joblib
import random
...
train_p_col = 'pitcher_id'  # 버그 수정: 원본은 'score_diff_pitcher_team'에 잘못 매칭됐음
...
print(f"  └ [검증] group by: {train_p_col}, 그룹 수: {train_df[train_p_col].nunique()}개", flush=True)
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups=groups)):
    ...
    print(f"  └ Fold {fold}: train pitchers={len(train_groups)}, val pitchers={len(val_groups)}, 겹침={len(overlap)}개", flush=True)
    ...
    base_cat = CatBoostClassifier(
        iterations=1500, learning_rate=0.01, depth=8, l2_leaf_reg=20.0,
        bagging_temperature=1.0, random_strength=1.0, border_count=32,
        random_seed=42+fold, verbose=0, thread_count=-1
    )
    base_cat.fit(X_train, y_train, eval_set=(X_val, y_val), early_stopping_rounds=100)
...
print("[6/6] v8(진짜 pitcher_id group k-fold + CatBoost 튜닝) 학습 완료!")
