"""sit_* 피처의 진짜 효과 재검증 (validation leak 수정판).

이전 v13/v13b/v14는 sit_stats(상황별 구종 비율 lookup)를 trackman_history 전체
(2019~2024)로 만들어서, 2024를 검증할 때 2024 자신의 패턴을 이미 알고 있는
상태였다 (연도 단위 정보 누출). 이번엔 sit_stats를 season<=2023 trackman
데이터만으로 만들어서, 실제 2025 제출 상황(trackman에 2025가 아예 없음)과
동일한 조건에서 재검증한다.
"""
import os
import random
...
print("[2/4] 상황 기반 구종 비율 집계 (trackman season<=2023만 사용, 2024 정보 배제)...")
tm_df = tm_df[tm_df['season'] <= 2023].copy()
print(f"  └ trackman 사용 행 수(<=2023): {len(tm_df)}")
...
print("[3/4] paired 비교 시작 (leak 수정판, season<=2023 trackman만 사용)...")
results = []
for drop_sit, label in [(True, 'baseline(sit_*없음)'), (False, 'sit_*포함(leak수정)')]:
...
for seed, d in by_seed.items():
    delta = d.get('sit_*포함(leak수정)', float('nan')) - d.get('baseline(sit_*없음)', float('nan'))
    print(f"  seed={seed}: baseline={d.get('baseline(sit_*없음)'):.1f} -> sit_*포함={d.get('sit_*포함(leak수정)'):.1f} (delta={delta:+.1f})")
