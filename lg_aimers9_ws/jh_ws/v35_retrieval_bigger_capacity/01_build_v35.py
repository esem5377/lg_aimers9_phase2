...
재사용(재학습 없음)하고, retrieval encoder만 "용량 확대" 버전으로 전체
프로덕션 데이터(2019~2024, 140만행)에서 처음부터 재학습.

배경(hackathon_fresh/retrieval_capacity_design/01_retrieval_bigger_capacity.py,
fold0/fold2 walk-forward): 용량 확대(EMBED_OUT_DIM 32->64, ENCODER_HIDDEN
[256,128]->[512,256], pitcher/batter_id 임베딩 24->48, team 6->12)가
fold0에서 solo bss_calibrated +151.97(2082.57->2234.54, skill_ratio
88.0%->94.4%) 개선, fold2에서는 -100.00(653.38->553.38, 77.0%->65.2%)
악화 -- 트레이드오프(두 폴드 부호 다름). 로컬 근거가 갈리지만 사용자가
실제 제출로 직접 확인하길 원해 프로덕션 빌드 진행.
...
calib carve-out(7.4만행, v34와 동일 split)에서 다시 그리드서치(1자유도).
script.py는 v26 것을 변경 없이 그대로 재사용 가능(RowEncoder 아키텍처를
...
V35_DIR = os.path.dirname(__file__)
V35_MODEL_DIR = os.path.join(V35_DIR, "model")
V35_OUT_DIR = os.path.join(V35_DIR, "output")

# ---- v25/train_final.py 함수 재사용(finalize_w07.py/01_build_v34.py와 동일 방식) ----
...
# ---- 용량 확대 아키텍처(retrieval_capacity_design/01과 동일) ----
EMBED_DIMS = {
    "pitcher_id": 48, "batter_id": 48,
    "pitcher_team_id": 12, "batter_team_id": 12,
...
def train_encoder(cat_train, x_num_train, y_train, cat_val, x_num_val, y_val, cardinalities, n_numeric,
    ...
    for epoch in range(start_epoch, MAX_EPOCHS):
        ...
        with torch.no_grad():
            ...
            # 메모리 안전을 위해 val 쿼리를 청크로 나눠 처리(계산 결과는 동일,
            # (n_val x ref_size) 유사도 행렬을 한 번에 만들지 않음 -- calib이
            # 커지면(프로덕션 7.4만행) 원본 방식이 수 GB 스파이크를 일으켜
            # OOM으로 학습이 중단되는 문제를 8/28 세션에서 실측으로 발견/수정).
        ...
        print(f"    epoch {epoch+1}/{MAX_EPOCHS} train_loss={epoch_loss/max(n_batches,1):.5f} "
              f"val_loss(ref={ref_size})={val_loss:.5f} temp={temp.item():.4f}", flush=True)
...
def main():
    print("=== v35: CatBoost(v26 재사용) + retrieval(용량확대, 전체 재학습) ===", flush=True)
    ...
    print("\n용량확대 retrieval encoder 학습(전체 프로덕션 데이터, 처음부터)...", flush=True)
    ...
    ckpt_path = os.path.join(V35_OUT_DIR, "encoder_train_checkpoint.pt")
    trained_encoder_path = os.path.join(V35_OUT_DIR, "encoder_trained_final.pt")
    os.makedirs(V35_OUT_DIR, exist_ok=True)
    t0 = time.time()
    ...
    print(f" best_w_catboost={best['w_catboost']:.2f}  calib_bss={best['calib_bss']:.2f}  "
          f"(참고: v26은 w=0.70, carve-out calibrated BSS=2082.78)", flush=True)

    # ---- 모델 아티팩트 저장 ----
    os.makedirs(V35_MODEL_DIR, exist_ok=True)
    os.makedirs(V35_OUT_DIR, exist_ok=True)

    torch.save(encoder.state_dict(), os.path.join(V35_MODEL_DIR, "retrieval_encoder.pt"))
    np.save(os.path.join(V35_MODEL_DIR, "reference_embeddings.npy"), z_ref.numpy())
    np.save(os.path.join(V35_MODEL_DIR, "reference_labels.npy"), y_ref.numpy())
    joblib.dump({"medians": medians, "scaler": scaler}, os.path.join(V35_MODEL_DIR, "numeric_prep.pkl"))

    import shutil
    shutil.copy(os.path.join(V26_MODEL_DIR, "catboost_seed42.cbm"), os.path.join(V35_MODEL_DIR, "catboost_seed42.cbm"))
    shutil.copy(os.path.join(V26_MODEL_DIR, "trackman_context.pkl"), os.path.join(V35_MODEL_DIR, "trackman_context.pkl"))
    ...
    with open(os.path.join(V35_MODEL_DIR, "feature_meta.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2, ensure_ascii=False)

    with open(os.path.join(V35_OUT_DIR, "metrics_v35.json"), "w", encoding="utf-8") as f:
        json.dump({
    ...
    print(f"\nSaved model artifacts to {V35_MODEL_DIR}", flush=True)
    print(f"Saved metrics to {V35_OUT_DIR}\\metrics_v35.json", flush=True)
