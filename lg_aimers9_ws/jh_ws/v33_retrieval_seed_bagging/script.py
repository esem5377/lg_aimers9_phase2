# script.py
# v26(CatBoost:retrieval=0.7:0.3, 1023점)에 시드 배깅 적용. CatBoost 6시드
# 평균 + retrieval(encoder도 시드별로 별도 학습) 6시드 평균을 각각 구한 뒤
# 0.7:0.3으로 블렌드, Platt calibration. 새 정보 추가 없이 순수 분산 감소
# (이 프로젝트에서 유일하게 두 번 검증된 성공 메커니즘, 8/21 CatBoost 단독
# 배깅 979/982점의 연장선).
import json
import os
...
def main():
    ...
    print(f" OK. seeds={seeds}  blend_weight_catboost={w_cb}  calibration={calib}")
    ...
    cat_query = encode_cats(test, meta_rt["cat_mappings"]) if len(test) else None
    x_num_query = prep_numeric_transform(test, meta_rt["numeric_cols"], medians, scaler) if len(test) else None
    ...
    for seed in seeds:
        ...
        cb_preds.append(cb_model.predict_proba(X_cb)[:, 1] if len(X_cb) else np.array([]))

        encoder = RowEncoder(
            meta_rt["cardinalities"], meta_rt["embed_dims"], meta_rt["encoder_hidden"],
            meta_rt["embed_out_dim"], 0.0, n_numeric,
        ).to(device)
        encoder.load_state_dict(torch.load(os.path.join(MODEL_DIR, f"retrieval_encoder_seed{seed}.pt"), map_location=device))
        encoder.eval()
        ...
        if len(test):
            retr_preds.append(retrieve_predict(encoder, device, cat_query, x_num_query, z_ref, y_ref))
        else:
            retr_preds.append(np.array([]))

    print("Build submission...")
    if len(test):
        cb_avg = np.mean(cb_preds, axis=0)
        retr_avg = np.mean(retr_preds, axis=0)
        blend_raw = w_cb * cb_avg + (1 - w_cb) * retr_avg
