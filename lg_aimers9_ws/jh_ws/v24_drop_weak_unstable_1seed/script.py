# script.py
# 987점(v22 drop_ingredients) 레시피에서 "약하면서 불안정한"(전체상관 매우
# 약함 + 연도간 부호 뒤집힘) asof_* 컬럼 5개(asof_pitcher_n/asof_batter_n/
# asof_pitcher_pitchmix_n/asof_pitcher_strike_rate/asof_pitcher_fastball_rate)
# 를 추가로 제거, 단일 시드(42). 나머지(trackman context 병합, raw id -1 OOV
# 인코딩, cat_cols 문자열화, Platt calibration)는 v18/v20/v22와 동일.
import json
import os
...
def main():
    ...
    test = add_risk_score_drop_cols(test)
