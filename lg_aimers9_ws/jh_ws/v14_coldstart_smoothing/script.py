# script.py
# 974점(es_ws v7) + asof_* cold-start(표본 0) NaN을 전역 평균(prior)으로
# 채우는 empirical Bayes smoothing 추가 버전. prior는 학습 때 train.csv
# 전체로 계산해 feature_meta.json에 저장해뒀고, test.csv에서도 동일한
# 값으로 채운다(test 자체 통계를 쓰지 않음 -- row-wise 독립 추론 원칙 준수).
import json
import os
...
def apply_coldstart_smoothing(df, meta):
    df = df.copy()
    for c, prior in meta["coldstart_priors"].items():
        if c in df.columns:
...
def main():
    ...
    print(f" OK. calibration={calib}  raw_id_cols={meta.get('raw_id_cols')}  "
          f"coldstart_cols={len(meta.get('coldstart_rate_cols', []))}")
    ...
    test = apply_coldstart_smoothing(test, meta)
