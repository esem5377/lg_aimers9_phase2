"""974점(es_ws v7) 레시피 그대로 + trackman_context 집계 전 METRIC_COLS
1%/99% winsorize만 추가(build_trackman_context_winsorized.py 참고).
그 외(CAT_COLS/RAW_ID_COLS/BEST_PARAMS/Platt calibration)는 전부 동일.

iterations=500 -- v12_rare_bucket과 동일 스케일로 비교하기 위해 고정
(8/19 확정: 974 baseline을 500으로 낮추면 923점. 이번 실험도 같은 500
스케일로 순수하게 winsorize 효과만 비교).
"""
import json
import os
...
TRACKMAN_CONTEXT_PATH = (
    r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v13_winsorize_trackman\model\trackman_context.pkl"
)
MODEL_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v13_winsorize_trackman\model"
OUT_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v13_winsorize_trackman\output"
...
def main():
    ...
    print("Load train data (winsorized trackman context)...")
    df = load_data()
    ...
    with open(os.path.join(OUT_DIR, "metrics_v13_winsorize.json"), "w", encoding="utf-8") as f:
