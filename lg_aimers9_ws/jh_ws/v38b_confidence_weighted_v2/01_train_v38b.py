"""v38b: v38(confidence-weighted CatBoost) 1차 시도가 carve-out에서 -2.48로
음수가 나온 뒤(K=100, floor=0.3), 공식 강도/범위를 조정해 재시도.

v1 사후분석: asof_pitcher_n/asof_batter_n 분포는 median~1776/2263, p10~226/247
인데 K=100은 이 분포에 비해 너무 작아서 하위 10%가량만 건드리고 나머지
90%+ 행은 weight가 이미 0.78~1.0에 몰려있었음(median weight=0.957) --
사실상 데이터의 극히 일부에만 약하게 작용한 셈인데도 음수가 나왔다는 건
"방향이 약해서 안 보였다"보다는 "방향 자체가 도움 안 됨" 쪽에 무게가 실림.
그래도 사용자가 공식 조정으로 한 번 더 확인을 요청해, K를 median 근처(2000)로
크게 올려 분포 전반(하위~중위 구간 전체)에 걸쳐 weight가 실질적으로 갈리도록
강도를 훨씬 세게 조정 -- "약해서 안 보였다" 가설을 이번엔 제대로 테스트.

가중치 공식(EB-GLMM의 n/(n+c) shrinkage 관례와 동일한 형태, K/floor만 변경):
  w_p = asof_pitcher_n / (asof_pitcher_n + K),  K=2000 (v1: 100)
  w_b = asof_batter_n / (asof_batter_n + K)
  sample_weight = 0.4 + 0.6 * sqrt(w_p * w_b)   (범위 [0.4, 1.0], v1: [0.3, 1.0])
  이제 median 행 weight~0.69, p10 행 weight~0.46로 광범위하게 갈림(v1은
  median 0.957로 사실상 거의 모두 1에 가까웠음).
