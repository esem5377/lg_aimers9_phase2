# script.py
# 987점 베이스(v22 control_risk_score+원재료제거)에 8/20 pruning(corr>0.95+LGBM
# importance==0 제거)을 적용한 버전. 단일 시드(42). meta["columns"]가 이미
# pruning 이후 최종 선택된 피처 목록이라, build_features가 그대로 슬라이싱하면
# 자동으로 pruning 반영됨(v22/v18과 동일 구조 재사용).
import json
import os
...
def build_features(df, meta):
    ...
    for c in meta["cat_cols"]:
        X[c] = X[c].astype(str)
    return X[meta["columns"]]  # pruning 이후 최종 선택된 피처만 남김
...
def main():
    ...
    print(f" OK. n_models={len(models)}  seeds={seeds}  n_features={len(meta['columns'])}  "
          f"calibration={calib}  raw_id_cols={meta.get('raw_id_cols')}")
    ...
    n_oov_pitcher = int((X["pitcher_id"] == -1).sum()) if "pitcher_id" in X.columns else -1
    n_oov_batter = int((X["batter_id"] == -1).sum()) if "batter_id" in X.columns else -1
    ...
    if len(X):
        ...
        raw_preds_avg = np.mean(raw_preds_per_seed, axis=0)
        preds = apply_calibration(raw_preds_avg, calib)
    else:
        preds = []
