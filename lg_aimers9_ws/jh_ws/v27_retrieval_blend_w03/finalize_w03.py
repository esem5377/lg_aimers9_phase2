"""v25/v26의 이미 학습된 모델(CatBoost/encoder/참조 임베딩)을 그대로
재사용, 블렌드 가중치만 0.3:0.7(CatBoost:retrieval)로 바꿔 calibration을
새로 fit. 재학습 없음.

주의: blend_weight_search_results.json 그리드서치에서 w_catboost=0.3(즉
retrieval=0.7)은 calibrated BSS 2034.43으로, v26(w=0.7, BSS 2082.78)이나
v25(w=0.5, BSS 2071.52)보다 로컬에서 이미 더 낮게 나온 지점. 사용자가 로컬
열세를 확인한 뒤에도 실제 제출까지 진행하기로 명시적으로 선택.
"""
import json
import os
...
V27_MODEL_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v27_retrieval_blend_w03\model"
V27_OUT_DIR = r"C:\Users\USER\Desktop\lg_aimers9_phase2\lg_aimers9_ws\jh_ws\v27_retrieval_blend_w03\output"
os.makedirs(V27_OUT_DIR, exist_ok=True)
...
cb_model.load_model(os.path.join(V27_MODEL_DIR, "catboost_seed42.cbm"))
...
numeric_prep = joblib.load(os.path.join(V27_MODEL_DIR, "numeric_prep.pkl"))
...
encoder.load_state_dict(torch.load(os.path.join(V27_MODEL_DIR, "retrieval_encoder.pt"), map_location=device))
encoder.eval()
z_ref = torch.tensor(np.load(os.path.join(V27_MODEL_DIR, "reference_embeddings.npy")), dtype=torch.float32)
y_ref = np.load(os.path.join(V27_MODEL_DIR, "reference_labels.npy"))
...
with open(os.path.join(V27_OUT_DIR, "metrics_v27.json"), "w", encoding="utf-8") as f:
    json.dump({
...
with open(os.path.join(V27_MODEL_DIR, "feature_meta.json"), "w", encoding="utf-8") as f:
    json.dump(meta, f, indent=2, ensure_ascii=False)
print(f"\nSaved feature_meta.json (blend_weight_catboost={W_CATBOOST}) to {V27_MODEL_DIR}", flush=True)
