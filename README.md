...
`lg_aimers9_ws/es_ws/open/data/*.csv`는 용량이 커서(train 352MB, trackman_history 338MB)
레포에는 포함되어 있지 않습니다. 대회 배포본에서 받아 `lg_aimers9_ws/es_ws/open/data/`
아래에 그대로 넣어주세요.
...
python3 lg_aimers9_ws/es_ws/work/pipeline/build_trackman_context.py

# 학습 (train.csv 필요, model/trackman_context.pkl 필요)
python3 lg_aimers9_ws/es_ws/work/pipeline/train.py

# 로컬 추론 확인 (test.csv 5건)
python3 lg_aimers9_ws/es_ws/work/pipeline/predict.py

# CatBoost 학습 (현재 최선 버전, model/trackman_context.pkl 필요)
python3 lg_aimers9_ws/es_ws/work/pipeline/train_catboost.py

# 제출 zip 재생성 (CatBoost, 현재 최선 — 학습 후 model_catboost/*를 submissions/submit_catboost/model/에 복사한 뒤)
cd lg_aimers9_ws/es_ws/work/submissions/submit_catboost && zip -r ../submit_v4_catboost.zip script.py model requirements.txt

# 제출 zip 재생성 (LightGBM, 이전 버전)
cd lg_aimers9_ws/es_ws/work/submissions/submit && zip -r ../submit_v2.zip script.py model requirements.txt
```
